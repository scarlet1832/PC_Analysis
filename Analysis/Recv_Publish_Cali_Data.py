...
class Recv_Publish:
    ...
    def connect_data_analysis_apply_bounding(self, point_cloud_nparray, fields):
        ...
        # Noise_all = len(points_all[:, 4]) / self.frame_counts
        # print("Noise Number:", Noise_all)

        # Noise_selected = len(points[:, 4]) / self.frame_counts
        # print("Noise selected Number:", Noise_selected)

        # Noise_within = points_all[np.where(points_all[:, 11] <= 600)]
        # self.Write_CSV(Noise_within, sorted_fields)
        # print("Noise_within Number:", len(Noise_within[:, 1])/self.frame_counts)
