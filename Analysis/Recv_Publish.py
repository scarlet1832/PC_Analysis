...
class Recv_Publish:
    def __init__(self):
        ...
        self.Topic = ["/rviz_selected_points", "/iv_points"]
        self.BoundingBox = []
        self.sub = None
        self.last_sign = 0
    ...
    def connect_data_analysis_get_bounding(self, point_cloud_nparray, fields):
        points, sorted_fields= self.analysis.extract.sort_fields(point_cloud_nparray, fields)
        self.BoundingBox = self.analysis.Get_Max_Min_xyz(points)
        # self.BoundingBox[0] -= 0.05
        # self.BoundingBox[1] += 0.15
        # self.BoundingBox[2] -= 0.15
        # self.BoundingBox[3] += 0.15
        print("Get boundingBox:", self.BoundingBox)
        # new things    
        # self.analysis.index = 2
        # POD = self.analysis.POD(points, 1, len(points[:, 4]))
        # Precision = self.analysis.fitting_plane.Extract_point_fitting_plane(points, [0,1], 'iv')
        self.sign = 1
        self.point_cloud_array = []
    ...
    def connect_data_analysis_apply_bounding(self, point_cloud_nparray, fields):
        points_all, sorted_fields= self.analysis.extract.sort_fields(point_cloud_nparray, fields)
    ...
    def process_data(self, pc_data):
        ...
        # elif self.topic == self.Topic[1]:
        #     fields = ["x", "y", "z", "intensity", "scan_id", "scan_idx"]
        #     print("Frame Number:", self.i)
        #     for point in pc_data:
        #         x, y, z, intensity, scan_id, scan_idx = point
        #         point_data = [x, y, z, intensity, scan_id, scan_idx]
        #         self.point_cloud_array.append(point_data)
        #     if self.i == self.frame_counts:
        #         point_cloud_nparray = np.array(self.point_cloud_array)
        #         self.connect_data_analysis_apply_bounding(point_cloud_nparray, fields)
    ...
    def Write_CSV(self, points, fields):
        ...
        np.savetxt(file_name, points, delimiter=",", header=",".join(column_headers), comments="", fmt="%d")
