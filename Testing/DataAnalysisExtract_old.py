...
class Extract:
    ...
    def get_file_data(self, file_path, topic, FrameLimit):
        ...
        res, sorted_fields = self.sort_fields(res, fields)
    ...
    def update_field_dict(self, sorted_fields):
        ...
        for i in range(len(sorted_fields)):
            for key in self.fields_index_dict:
                if sorted_fields[i] == key:
                    self.fields_index_dict[key] = i
        # 处理 'flag' 键
        ...
        for key, value in self.fields_index_dict.items():
...
class Analysis:
    ...
    def POD(self, pts, frame_count, real_points):
        ...
        print(self.Horizontal_R[1])
        row = math.atan(Width / (2 * distance)) * 2 * 180 / math.pi / self.Horizontal_R[1]
        # row = math.floor(row)
        col = math.atan(Height / (2 * distance)) * 2 * 180 / math.pi / self.Vertical_R[1]
        col = 48
